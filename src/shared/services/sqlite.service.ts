...
export class SqliteService {
    ...
    createThreads() {
        var self = this;
        self.db.executeSql('CREATE TABLE IF NOT EXISTS Threads ( key VARCHAR(255) PRIMARY KEY NOT NULL, title text NOT NULL, question text NOT NULL, category text NOT NULL, datecreated text, USER VARCHAR(255), comments INT NULL, votesUp INT NULL, votesDown INT NULL);', {}).then(() => {
        }, (err) => {
    ...
    addThread(thread: IThread) {
        ...
        let query: string = 'INSERT INTO Threads (key, title, question, category, datecreated, user, comments, votesUp, votesDown) VALUES (?,?,?,?,?,?,?,?,?)';
