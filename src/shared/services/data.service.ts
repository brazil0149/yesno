...
export class DataService {
    ...
    removePost(post: string) {
        return this.threadsRef.child('threads').remove(post);
    }
    ...
    votePost(postKey: string, like: boolean, user: string): any {
        let postRef = this.threadsRef.child(postKey + '/votes/' + user);
        return postRef.set(like);
    }
